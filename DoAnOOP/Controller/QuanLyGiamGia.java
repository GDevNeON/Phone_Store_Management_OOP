...
public class QuanLyGiamGia extends Discount implements ControllerInterface {
    ...
    public Discount[] getListGiamGia() {
        ...
        DSGG = new Discount[result.length];
        for (int i = 0; i < result.length; i++) {
            String[] row = result[i].split(";");
            DSGG[i] = new Discount(row[0], row[1], row[2], row[3], row[4], row[5], row[6]);
        }
        return DSGG;
    }
    ...
    public Discount[] addGiamGia(Discount[] DSGG, Discount giamGia) {
        DSGG = Arrays.copyOf(DSGG, DSGG.length + 1);
        DSGG[DSGG.length - 1] = giamGia;
        return DSGG;
    }

    public Discount[] deleteGiamGia(Discount[] DSGG, int index) {
        Discount[] newGg = new Discount[DSGG.length - 1];
